...
contract RandomNumber {
        ...
        payable
    {
        ...
        require(
            msg.value > price * _numberOfTimes,
            "There already is a higher bid."
        );
        allprice = allprice + msg.value;
        ...
        for (uint256 i = 0; i < _numberOfTimes; i++) {
            ...
            if (randomNumber[i] <= normal * quantity1) {
                quantity1 = quantity1 - 1;
                prize = stringConnect(prize, "Normal,");
            } else if (
                randomNumber[i] > normal * quantity1 &&
                randomNumber[i] <= normal * quantity1 + rare * quantity2
            ) {
                ...
                randomNumber[i] > normal * quantity1 + rare * quantity2 &&
                randomNumber[i] <=
                normal * quantity1 + rare * quantity2 + ultimate * quantity3
            ) {
    ...
    {
        return (price, maxNumber, quantity1, quantity2, quantity3);
    }
